...
class TestPhonemeMouth(unittest.TestCase):
    def test_vault_uses_fv_cell_not_spelling_vowel(self):
        with tempfile.TemporaryDirectory() as tmp:
            sheet_path = Path(tmp) / "leo.png"
            ensure_placeholder_sheet(sheet_path, "leo")
            sheet = CharacterSheet(sheet_path)
            phones = build_phone_intervals("vault", 1.0)
            line = {
                "id": 3,
                "text": "vault",
                "duration": 1.0,
    ...
    def test_map_uses_mbp_cell_not_pause(self):
        with tempfile.TemporaryDirectory() as tmp:
            sheet_path = Path(tmp) / "leo.png"
            ensure_placeholder_sheet(sheet_path, "leo")
            sheet = CharacterSheet(sheet_path)
            phones = build_phone_intervals("map", 1.0)
            m_seg = next(p for p in phones if p["phone"] == "M")
            line = {"id": 1, "text": "map", "duration": 1.0, "phones": phones}
            t_m = (float(m_seg["t0"]) + float(m_seg["t1"])) / 2
            frame_path = _frame_for_line(sheet, line, t_m)
            actual = Image.open(frame_path).getpixel((0, 0))
            self.assertEqual(actual, sheet.mbp().getpixel((0, 0)))
