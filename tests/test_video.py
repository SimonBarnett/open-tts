...
class TestPhonemeMouth(unittest.TestCase):
    def test_vault_uses_fv_not_spelling_vowel(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            ensure_placeholder_sheet(sheet_path, "leo")
            sheet = CharacterSheet(sheet_path)
            track = build_phone_intervals("vault", 1.0)
            line = {
                "id": 3,
                "text": "vault",
                "duration": 1.0,
    ...
    def test_map_uses_mbp_cell(self):
        with tempfile.TemporaryDirectory() as tmp:
            sheet_path = Path(tmp) / "leo.png"
            ensure_placeholder_sheet(sheet_path, "leo")
            sheet = CharacterSheet(sheet_path)
            track = build_phone_intervals("map", 1.0)
            m_seg = next(p for p in track if p["phone"] == "M")
            t_m = float(m_seg["t0"]) + 0.01
            line = {"id": 1, "text": "map", "duration": 1.0, "phones": track}
